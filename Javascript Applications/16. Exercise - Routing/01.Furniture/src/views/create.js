...
export function createPage(ctx) {
    ...
    async function onSubmit(event) {
        ...
        const formData = [...(new FormData(event.target)).entries()]; // масив от масиви
        const data = formData.reduce((a, [k, v]) => Object.assign(a, {[k]: v.trim() }), {}); // обект с всички полета
        // console.log(data);
        const missing = formData.filter(([k, v]) => k != 'material' && v.trim() == ''); // списък с празните полета
