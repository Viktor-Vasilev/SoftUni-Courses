...
namespace _20200628_02._Snake
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            while (true)
            {
               ...
               // Print(matrix);
                ...
                if (matrix[snakeRow, snakeCol] == '*') // ако срещнем храна
                {
                    ...
                    if (foodEaten == 10) // проверяваме дали трябва да приключим
                    {
                ...
                if (matrix[snakeRow, snakeCol] == 'B') // ако влезем в леговище
                {
                    matrix[snakeRow, snakeCol] = '.';   // маркираме текущото местопожение
                    ...
                    {                        
                        ...
                        {                          
                            if (matrix[row, col] == 'B') // взимаме си местоположението на другото леговище
                            {
                                snakeRow = row;
                                snakeCol = col;
                            }
            ...
            if (foodEaten == 10) // ако сме я нахранили
            {
        ...
        private static void FillMatrix(int size, char[,] matrix, ref int snakeRow, ref int snakeCol)
        {
            for (int row = 0; row < size; row++)
            {
                ...
                for (int col = 0; col < size; col++)
                {
                    ...
                    if (input[col] == 'S') // взимаме си местоположението на змията
                    {
                        snakeRow = row;
                        snakeCol = col;
                    }
